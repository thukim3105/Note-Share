...
export function Admin() {
  ...
  const callWithLoading = useCallback(async (fn, ...args) => {
    setLoading(true);
    try {
  ...
  const fetchUsers = useCallback(
    ...
    [callWithLoading, searchEmail, searchName, token],
  );
  ...
  const handleLogin = async (event) => {
    ...
    try {
      const result = await callWithLoading(loginAdmin, {
  ...
  const handleUserSave = async (event) => {
    ...
    try {
      if (selectedUser) {
        await callWithLoading(updateUser, selectedUser.id, payload, token);
        setStatusMessage("User updated successfully.");
      } else {
        await callWithLoading(createUser, payload, token);
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      if (action === "delete") {
        await callWithLoading(deleteUser, user.id, token);
        setStatusMessage(`User ${user.email} soft deleted.`);
      } else if (action === "restore") {
        await callWithLoading(restoreUser, user.id, token);
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await callWithLoading(bulkDeleteUsers, ids, token);
  ...
  const handleBulkCreate = async () => {
    try {
      ...
      await callWithLoading(bulkCreateUsers, parsed, token);
  ...
  const handleBulkUpdate = async () => {
    try {
      ...
      await callWithLoading(bulkUpdateUsers, parsed, token);
  ...
  if (!token) {
    return (
      <div className="grid min-h-[calc(100vh-88px)] place-items-center">
        <div className="w-full max-w-lg rounded-3xl border border-[#e8e6dc] bg-white p-8 shadow-sm">
          <div className="space-y-4">
            <div className="space-y-3">
              ...
              <Text className="text-sm text-[#5e5d59]">
                Use admin credentials to authenticate and access the user
                management API.
              </Text>
            </div>
            <AdminLoginForm
            ...
            />
            {statusMessage ? (
              <Text className="text-sm text-[#4d4c48]">{statusMessage}</Text>
            ) : null}
          </div>
        </div>
      </div>
      ...
      {activeTab === "directory" ? (
          ...
          <Card className="space-y-6 bg-[#ffffff] p-6">
            ...
            <DataTable
              ...
              renderRow={({ row, selected, onToggleRow }) => [
                ...
                <td key="email" className="p-3 align-top text-[#4d4c48]">
                  {row.email || "—"}
                </td>,
                ...
                <td key="actions" className="p-3 align-top">
                  <DropDown
                    label="Actions"
                    items={[
                      {
                      ...
                      },
                      {
                        ...
                        onClick: () =>
                          setConfirmModal({
                            open: true,
                            action: row.is_deleted ? "restore" : "delete",
                            user: row,
                          }),
                      },
                    ]}
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="text-sm text-[#5e5d59]">
                {selectedIds.length} selected for bulk delete.
              </div>
              <Button
                type="button"
                ...
                onClick={() => {
                  if (!selectedIds.length) {
                    setStatusMessage(
                      "Select at least one user to bulk delete.",
                    );
                    return;
                  }
        ...
        <Card className="space-y-6 bg-[#ffffff] p-6">
          <div className="space-y-4">
            ...
            <Text className="text-sm text-[#5e5d59]">
              Send complex requests through three bulk endpoints. Use raw JSON
              for bulk create and bulk update, or provide a list of numeric IDs
              for bulk delete.
            </Text>
          </div>
          ...
          <div className="grid gap-6 lg:grid-cols-3">
            <div className="space-y-3 rounded-3xl border border-[#f0eee6] bg-[#faf9f5] p-4">
              ...
              <Text className="text-sm text-[#5e5d59]">
                Comma-separated user IDs for soft delete.
              </Text>
              <textarea
                value={bulkDeleteIds}
              ...
              <Button
                type="button"
                className="w-full bg-[#b53333] hover:bg-[#9b2b2b]"
            ...
            <div className="space-y-3 rounded-3xl border border-[#f0eee6] bg-[#faf9f5] p-4 lg:col-span-2">
              ...
              <Text className="text-sm text-[#5e5d59]">
                Paste an array of user objects and send to bulk create endpoint.
              </Text>
              <textarea
          ...
          <div className="space-y-3 rounded-3xl border border-[#f0eee6] bg-[#faf9f5] p-4">
            ...
            <Text className="text-sm text-[#5e5d59]">
              Provide an array of update objects for the bulk update endpoint.
            </Text>
            <textarea
            ...
            <Button
              type="button"
              className="w-full bg-[#141413] hover:bg-[#30302e]"
